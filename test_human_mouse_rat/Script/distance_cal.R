...
perc = function(filename){
  ...
  identical_pos3 = length(which(spec.2[spec.2 == spec.3] != '-'))
 
  
  id_pos = c(identical_pos1,identical_pos2,identical_pos3)
  ...
  shared_gap3 = length(which(spec.2[spec.2 == spec.3] == '-'))
  ...
  aligned.all = comb.l - c((l1+l2-shared_gap1),(l1+l3-shared_gap2),(l2+l3-shared_gap3))
...
distance_plot= function(dir,ouFile,ouFig){
  ...
  plot(density(spec.2_3),xlab="percent sequence identity",ylab="freq",
  ...
  legend("topleft",legend = c("mouse-rat","human-mouse","human-rat"),
