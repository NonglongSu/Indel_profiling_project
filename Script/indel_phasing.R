...
main = function(dir, ouFile, ouFig, num){
  ...
  # Draw a figure
  pdf(ouFig)    
  Pdf      = as.table(noquote(t(Phase.df)))
  col.name = as.matrix(Pdf[1, ])
  Pdf      = Pdf[-1, ]
  colnames(Pdf) = col.name
  
  barplot(Pdf, main = "The proportion of indel phases across length of gaps", 
          xlab = paste0("Gap length (window size = ", Window, ")"), ylab = "Count", col = 1:3 ) 
  legend("topleft", bg ="transparent", bty = "n", rownames(Pdf), fill = 1:3, cex = 0.75)
