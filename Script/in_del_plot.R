...
main = function(dir1, dir2, ouFig, Name, num){
  ...
  barplot(Phase.m, xlab = Name[1], ylab = "Count", col = 1:3 ) 
  barplot(Phase.r, xlab = Name[2], ylab = "Count", col = 1:3 ) 
  
  Window = as.numeric(num)
  mtext("The proportion of indel phases across length of gaps", outer = TRUE, cex = 1.5)
  mtext(paste0("Gap length (window size = ", Window, ")"), outer = TRUE, cex = 1.5, side = 1)
  legend(2.5, 800, bg ="transparent", bty = "n", legend = rownames(Phase.m), fill = 1:3, cex = 0.75)
